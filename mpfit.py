  ...
  Keywords:
   ...
   Outputs:
	 ...
	 .perror
		...
		*If* you can assume that the true reduced chi-squared value is unity --
		...
		if parinfo is not None:
			if type(parinfo) != list: #from python2 to python3, types.listtype to list(yibo)
		...
		if (limited is not None) and (limits is not None):
			...
			#	print(limits[:,0],limits[:,1]) #this line was added by Yibo to check a bug
	...
	def calc_covar(self, rr, ipvt=None, tol=1.e-14):
		...
		if numpy.ndim(rr) != 2:  # I have change the old expression 'numpy.rank' to this (Yibo)
